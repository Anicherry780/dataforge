...
def deduplicate(records: List[Dict], key: str = "order_id") -> Tuple[List[Dict], int]:
    seen: set = set()
    unique = []
    for r in records:
        k = r.get(key)
...
# ── Order-pipeline public helpers ───────────────────────────────────────────────

_REQUIRED_ORDER_FIELDS = {"order_id", "user_id", "product_id", "amount", "quantity", "country", "status"}


def cast_types(records: List[Dict]) -> List[Dict]:
    """Cast order record fields to their expected types."""
    for r in records:
        amount = r.get("amount")
        if amount is None:
            r["amount"] = 0.0
...
def filter_invalid(records: List[Dict]) -> Tuple[List[Dict], int]:
    """Drop records missing required fields or with empty key values."""
    valid = []
    for r in records:
        if all(r.get(f) not in (None, "") for f in _REQUIRED_ORDER_FIELDS):
            valid.append(r)
    return valid, len(records) - len(valid)


def enrich(records: List[Dict]) -> List[Dict]:
    """Add derived fields: revenue, unit_price, day_of_week, hour_of_day."""
    _days = ["Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday"]
    for r in records:
        amount = r.get("amount") or 0.0
        quantity = r.get("quantity") or 1
        r["revenue"] = round(amount * quantity, 2)
        r["unit_price"] = round(amount / quantity, 2) if quantity else 0.0
        try:
            ts_str = str(r.get("created_at", "")).replace("Z", "+00:00")
            ts = datetime.fromisoformat(ts_str)
            r["day_of_week"] = _days[ts.weekday()]
            r["hour_of_day"] = ts.hour
        except Exception:
            now = datetime.now(timezone.utc)
            r["day_of_week"] = _days[now.weekday()]
            r["hour_of_day"] = now.hour
        r.setdefault("processed_at", _now())
    return records
...
def detect_anomalies(records: List[Dict]) -> List[Dict]:
    ...
    for r in records:
        ...
        r["is_anomaly"] = z > 3.5
        r["anomaly_reason"] = f"z-score={z:.1f}" if z > 3.5 else ""
    return records


def compute_aggregations(records: List[Dict]) -> Dict[str, Any]:
    """Compute summary metrics over order records (must already have revenue field)."""
    revenues = [r.get("revenue") or 0.0 for r in records]
    total = sum(revenues)
    return {
        "total_records": len(records),
        "total_revenue": round(total, 2),
        "avg_order_value": round(total / len(records), 2) if records else 0.0,
        "anomalies_detected": sum(1 for r in records if r.get("is_anomaly")),
    }
